 ...
 protected:
  void initialize(const std::string strategy, bool verbose = true) {
    ...
    for (size_t threadId = 0; threadId < numThreads; ++threadId) {
      threadToNumaNode[threadId] =
          numa_node_of_cpu(threadToLogicalCpu[threadId]);
  ...
  inline void distributeRoundRobin() {
    ...
    for (size_t t = 0; t < numThreads; ++t) {
      ...
      while (currentLogicalCpuIndex[currentNumaNode] >=
             numaNodeToLogicalCpus[currentNumaNode].size())
        currentNumaNode = (currentNumaNode + 1) % numNumaNodes;

      threadToLogicalCpu[t] =
          numaNodeToLogicalCpus[currentNumaNode]
