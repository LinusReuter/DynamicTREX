...
inline void validateGenerators(const TripBased::Transfers& genA, const TripBased::Transfers& genB,
    ...
    if (diff.areEqual()) {
        ...
        std::cout << "Missing from B: " << diff.onlyInFirst.size() << " edges.\n";
    } else if (diff.isSecondSuperset()) {
        std::cout << "Generator B is a strict superset of Generator A.\n";
        std::cout << "Missing from A: " << diff.onlyInSecond.size() << " edges.\n";
    } else {
    ...
    if (!diff.onlyInFirst.empty()) {
        ...
        for (size_t i = 0; i < std::min(diff.onlyInFirst.size(), size_t(10)); ++i) {
    ...
    if (!diff.onlyInSecond.empty()) {
        ...
        for (size_t i = 0; i < std::min(diff.onlyInSecond.size(), size_t(10)); ++i) {
...
inline TripBased::Transfers exportStaticTransfersForTopologyComparison(const TripBased::Data& data) {
    std::vector<Edge> beginOut = data.stopEventGraph.getBeginOut();
    std::vector<TripBased::EdgeLabel> labels(data.stopEventGraph.numEdges());
    std::vector<int> travelTime(data.stopEventGraph.numEdges(), 0);
...
public:
    ...
    virtual void execute() noexcept override {
        ...
        const TripBased::Transfers staticInitialTransfers = exportStaticTransfersForTopologyComparison(staticTripData);
